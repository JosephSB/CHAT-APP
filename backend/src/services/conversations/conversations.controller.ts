...
import { createConversation, existConversationBetweenUsers, findConversationsOfUser, findConversationWithID, findMessagesOfConversation, userBelongsToConversation } from "./conversations.model";
...
export const getConversations = async (req: Request, res: Response) => {
    ...
    try {
        //traer mensajes
        const resp = await findConversationsOfUser(user?.user_id || "").catch(() => { throw new Error("Error al traer conversaciones") })

        res.status(200).json({
            message: "tus conversaciones",
            data: resp || []
        })
